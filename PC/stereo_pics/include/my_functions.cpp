...
int snaps (bool skip)
{
       ...
       // VideoCapture capR(5),capL(4);//zed

        capR.set(CAP_PROP_FRAME_HEIGHT, 960);
        capR.set(CAP_PROP_FRAME_WIDTH, 1280);
        //capR.set(CAP_PROP_FPS, 60);
        capL.set(CAP_PROP_FRAME_HEIGHT, 960);
        ...
        while(key != 'q') {
            ...
            if (key == 'c' && skip == false) {
                ...
                imwrite(l, frameL);
               //****
                string dossier_temp = "../../stereo_calib2/data";
                r = dossier_temp + "/calib_image_droite/right"+inttostr(count)+".jpg";//a def
                l = dossier_temp + "/calib_image_gauche/left"+inttostr(count)+".jpg";// a def
...
//-------------------------------------------------------
static void StereoCalib(bool skip, const vector<string>& imagelist, Size boardSize,bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
...
int stereo_match(int argc, char** argv)
{
    ...
    if(intrinsic_filename)
    {
        ...
        imwrite("imageGRect.jpg", img1r);
        imwrite("imageDRect.jpg", img2r);
    }
